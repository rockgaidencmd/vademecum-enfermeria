...
function navigateTo(view) {
  ...
  // Ocultar todas las vistas (limpiar AMBAS clases)
...
function navigateToMed(medId) {
  ...
  // Mostrar vista medicamento
  const viewMed = $('view-med');
  if (viewMed) viewMed.classList.add('active');
  ...
  if (!State._skipHistory) {
    history.pushState({ view: 'med', medId }, '', '#med-' + medId);
  }
}
...
function showCategoryMeds(catId) {
  ...
  // Vista dedicada de resultados de categoría
  const viewCat = $('view-cat-meds');
  const content = $('catMedsContent');
  viewCat.classList.add('active');

  content.innerHTML = `
    <div class="view-header">
      <button class="med-header-back" id="catBackBtn" aria-label="Volver" style="margin-bottom:10px;">‹ Categorías</button>
  ...
  const backBtn = $('catBackBtn');
  if (backBtn) backBtn.addEventListener('click', () => navigateTo('categorias'));
...
function renderMedCard(med) {
  ...
  return `
    ...
    <div class="med-branding">
      ...
      <div class="med-branding-cross" style="display:none;"><span></span><span></span></div>
...
/* ═══════════════════════════════════════════════════════
   HISTORIAL — Botón atrás del celular
═══════════════════════════════════════════════════════ */
function bindHistory() {
  // Estado inicial: home en el historial (para que el primer "atrás" salga de la app)
  history.replaceState({ view: 'home' }, '', '#home');

  window.addEventListener('popstate', e => {
    const st = e.state || { view: 'home' };
    State._skipHistory = true; // Evita re-pushear al restaurar

    if (st.view === 'med' && st.medId) {
      navigateToMed(st.medId);
    } else if (st.view === 'cat-meds' && st.catId) {
      showCategoryMeds(st.catId);
    } else {
      navigateTo(st.view || 'home');
    }
